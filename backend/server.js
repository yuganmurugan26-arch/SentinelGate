...
const FACULTY_INVITE_CODE = 'IRUMBU THIRAI'; // ← change this to your real code whenever you like
...
function toggleInviteField(){
  ...
  const field = document.getElementById('reg-invite-field');
  if(role === 'Faculty'){ field.classList.remove('hidden'); }
  else { field.classList.add('hidden'); document.getElementById('reg-invite-code').value=''; }
}
...
const OTP_LIFETIME_SECONDS = 180; // 3 minutes — must match backend OTP_TTL_MS
let otpTimer=null;
...
async function handleRegister(){
  ...
  if(inviteCode !== FACULTY_INVITE_CODE){
    err.innerHTML = errHtml('Invalid staff invite code. Contact your administrator for the correct code.');
    addAlert('Medium', 'Failed faculty registration attempt', `"${username}" tried to register as Faculty with an incorrect invite code.`);
    return;
  }
}
